(function() {
  ...
  _.last = function(array, n) {
    return n === 0 ? [] : 
      ...
      n === undefined ? array[array.length -1] : array.slice(n -1);
  };
  ...
  _.flatten = function(nestedArray, result) {
    var flatten = [];
    (function isNested(nestedArray) {
      for (var i = 0; i < nestedArray.length; i++) {
        if (Array.isArray(nestedArray[i])) {
          isNested(nestedArray[i]);
        } else {
          flatten.push(nestedArray[i]);
        }
      }
    }) (nestedArray);
    return flatten;
  ...
  _.intersection = function() {
    ...
    var reduce = _.reduce(arr, function(arr1, arr2) {
      return arr1.concat(arr2)
    });
    for (var j = 0; j < arr.length; j++) {
      reduce = _.filter(reduce, function(element) {
        return _.indexOf(arr[j], element)!== -1;
      });
    }
    return Array.from(new Set(reduce));
  };
